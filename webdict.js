...
function fetchDefiniton (site, word) {
  webdict(site, word)
        .then(resp => {
          if (resp.message === 'success') {
            ...
            resp.definition.forEach((val, idx) => {
              console.log(chalk.bold('Defintion', idx + 1) + ':\n' + chalk.green(val))
            })
          } else {
            console.log(chalk.red(`${resp.message}`))
          }
        })
}
...
if (program.dictionary) {
  fetchDefiniton('dictionary', program.dictionary)
} else if (program.urbandictionary) {
  fetchDefiniton('urbandictionary', program.urbandictionary)
} else {
